...
class QFDCosmology:
    ...
    def comoving_distance(self, redshift: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        ...
        if isinstance(redshift, (int, float)):
            ...
            result[nonlinear_mask] = (self.c / self.H0) * (z_nl + 0.5 * z_nl**2)
    ...
    def critical_density(self, redshift: Union[float, np.ndarray] = 0) -> Union[float, np.ndarray]:
        ...
        H_z = self.H0 * 1.022e-12 * (1 + redshift)**1.5  # H(z) for matter-dominated
        
        return 3 * H_z**2 / (8 * np.pi * G)
    
    def sound_horizon(self, redshift: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        """
        ...
        # r_s ≈ c/(H0 * sqrt(Ω_m)) * 2/sqrt(1+z_eq) * ln[(sqrt(1+z_eq) + sqrt(z_eq))/(1+sqrt(z_eq))]
        ...
        if isinstance(redshift, (int, float)):
            ...
            result[low_z_mask] = prefactor * np.sqrt(z_eq / (1 + redshift[low_z_mask]))
