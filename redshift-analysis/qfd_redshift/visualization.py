...
class RedshiftPlotter:
    ...
    def plot_hubble_diagram(
        self, hubble_data: Dict, save_path: Optional[Union[str, Path]] = None
    ) -> None:
        """
        ...
        ax1.plot(
            redshifts,
            m_qfd,
            color=self.colors["qfd"],
            linewidth=3,
            label="With QFD Dimming",
            marker="o",
            markersize=3,
            alpha=0.8,
        )
        ...
        ax1.set_title(
            "QFD Hubble Diagram", fontsize=14, fontweight="bold"
        )
        ...
        ax2.set_title(
            "QFD Dimming Evolution", fontsize=14, fontweight="bold"
        )
    ...
    def plot_qfd_vs_lambda_cdm(
        self, comparison_data: Dict, save_path: Optional[Union[str, Path]] = None
    ) -> None:
        """
        ...
        ax1.set_ylabel("Apparent Magnitude", fontsize=12)
        ax1.set_title(
            "QFD vs ΛCDM Comparison", fontsize=14, fontweight="bold"
        )
        ax1.legend(fontsize=11)
        ax1.grid(True, alpha=0.3)
        ax1.invert_yaxis()
        ...
        ax2.set_xlabel("Redshift z", fontsize=12)
        ax2.set_ylabel(
            "Magnitude Difference (QFD - ΛCDM)", fontsize=12
        )
        ax2.set_title(
            "Model Differences", fontsize=14, fontweight="bold"
        )
        ax2.grid(True, alpha=0.3)
    ...
    def plot_validation_results(
        self, validation_data: Dict, save_path: Optional[Union[str, Path]] = None
    ) -> None:
        """
        ...
        ax2.set_xlabel("Redshift z", fontsize=12)
        ax2.set_ylabel(
            "Residual (Predicted - Observed)", fontsize=12
        )
        ax2.set_title(
            "Model Residuals", fontsize=14, fontweight="bold"
        )
        ax2.grid(True, alpha=0.3)
    ...
    def plot_comprehensive_analysis(
        self, results: Dict, save_path: Optional[Union[str, Path]] = None
    ) -> None:
        """
        ...
        # Add overall title
        fig.suptitle(
            "QFD Redshift Analysis: Alternative to Dark Energy\n"
    ...
    def plot_physics_scaling(
        self, physics_data: Dict, save_path: Optional[Union[str, Path]] = None
    ) -> None:
        """
        ...
        ax1.plot(
            redshifts,
            physics_data["qfd_coupling_evolution"],
            color=self.colors["qfd"],
            linewidth=2,
        )
        ax1.set_xlabel("Redshift z")
        ...
        ax2.semilogy(
            redshifts, physics_data["igm_density_evolution"], color="green", linewidth=2
        )
        ax2.set_xlabel("Redshift z")
        ...
        # Path length effects
        ax3.loglog(
            redshifts, physics_data["path_length_effects"], color="orange", linewidth=2
        )
        ax3.set_xlabel("Redshift z")
        ...
        # Total optical depth
        ax4.semilogy(
            redshifts, physics_data["total_optical_depth"], color="purple", linewidth=2
        )
        ax4.set_xlabel("Redshift z")
    ...
    def create_publication_figure(
        ...
        gs = gridspec.GridSpec(3, 3, hspace=0.4, wspace=0.3, height_ratios=[1, 1, 0.6])
        ...
        ax_main.plot(
            hubble_data["redshifts"],
            hubble_data["magnitudes_standard"],
            "k--",
            linewidth=2,
            label="Standard Candle",
            alpha=0.7,
        ...
        # Add physics caption
        caption = (
            "QFD provides alternative explanation for cosmological dimming "
    ...
    def _add_side_panels(self, fig, gs, results):
        ...
        stats_text = (
            ...
            f"• RMS Error vs Observations: {rms_error:.3f} magnitudes\n"
            f"• RMS Difference vs ΛCDM: {rms_lambda_cdm:.3f} magnitudes\n"
            f"• Validation Passed: {validation['validation_passed']}\n"
            "• No Dark Energy Required: Physics-based alternative using "
            "E144-validated interactions"
        )
