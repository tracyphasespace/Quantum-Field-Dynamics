...
def numpyro_model_v17(z_obs, m_obs, A_plasma, beta):
    ...
    eta_prime = numpyro.sample('eta_prime', dist.Normal(0.0, 0.1))
    xi = numpyro.sample('xi', dist.Normal(0.0, 0.1))
    sigma_m = numpyro.sample('sigma_m', dist.HalfNormal(0.2).mask(False)) + 1e-6
    M_abs = numpyro.sample('M_abs', dist.Normal(-19.3, 1.0))
    ...
    def process_single_sn(z_obs_single, A_plasma_single, beta_single):
        ...
        m_pred = predict_apparent_magnitude(xi, distance, z_drag, z_local, M_abs)
        return m_pred
...
def main(args):
    ...
    init_params = {'k_J': 70.0, 'eta_prime': 0.0, 'xi': 0.0, 'sigma_m': 0.2, 'M_abs': -19.3}
    kernel = NUTS(
        numpyro_model_v17,
