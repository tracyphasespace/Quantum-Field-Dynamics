...
class Phase8Model(torch.nn.Module):
    ...
    def temporal_penalty(self, rho_total: torch.Tensor) -> torch.Tensor:
        """
        Mass-dependent temporal stiffness penalty.
        """
        if self.tau_scale == 0.0:
            return rho_total.new_tensor(0.0)
        coeff = self.tau_scale * (self.A ** self.tau_power)
        return coeff * (rho_total * rho_total).sum() * self.dV
    ...
    def energies(self) -> Dict[str, torch.Tensor]:
        ...
        V_temporal = self.temporal_penalty(rhoN + rhoE)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--tau-scale", type=float, default=0.0,
                   help="Temporal quagmire strength (0 to disable).")
    p.add_argument("--tau-power", type=float, default=1.0,
                   help="Mass scaling exponent for temporal penalty (default 1.0).")
...
def main() -> None:
    ...
    try:
        ...
        payload = {
            ...
            "tau_power": float(args.tau_power),
            "alpha_model": args.alpha_model,
        ...
        payload.update({k: energy_terms.get(k) for k in [
            ...
            "V_iso","V_rotor","V_surf","V_coul_cross","V_mass_N","V_mass_e","V_sym","V_temporal"
        ]})
