...
def run_qfd_solver(A: int, Z: int, params: Dict, verbose: bool = False, retry: bool = True,
                   fast_mode: bool = True, device: str = "cpu", early_stop_vir: float = 0.18) -> Dict:
    """
    Run QFD solver with given parameters (DIRECT CALL - NO SUBPROCESS).

    PERFORMANCE FIX (2025-12-31): Eliminates subprocess overhead (~3x faster).
    - Before: subprocess.run() → ~1s Python startup per isotope
    - After: Direct import → 0s startup, pure solve time
    ...
    Args:
        ...
        early_stop_vir: Stop early when |virial| < this threshold (default 0.18)
    """
    import sys
    import torch
    from pathlib import Path
    ...
    try:
        # Setup (matches qfd_solver.py main() exactly)
        seed = 4242
        torch_det_seed(seed)
        device_obj = torch.device(device if (device == "cpu" or torch.cuda.is_available()) else "cpu")

        # Rotor params (qfd_solver.py defaults from argparse)
        rotor = RotorParams(
            lambda_R2=3e-4,
            lambda_R3=1e-3,
            B_target=0.01
        ...
        # Coulomb coupling (matches qfd_solver.py default=1.0, NOT fine structure constant!)
        # BUG FIX: Was incorrectly using 1/137.036, causing huge energy errors
        alpha_coul = 1.0
        ...
        # Initialize fields (matches qfd_solver.py)
        ...
        # Extract results (matches qfd_solver.py JSON payload)
        with torch.no_grad():
            E_model = float(best_result["E"])
            virial_val = float(virial)
            virial_abs = float(abs(virial))

            # Physical success check (matches qfd_solver.py)
            physical_success = (virial_abs < 0.5 and E_model < 0)

            # Build data structure matching subprocess version
            data = {
                "status": "ok",
                "A": int(A),
                "Z": int(Z),
        ...
        if verbose:
            if physical_success:
                print(f"✓ E={E_model:.2f} MeV, vir={virial_abs:.3f}")
            else:
                print(f"⚠ E={E_model:.2f} MeV, vir={virial_abs:.3f} (not converged)")
