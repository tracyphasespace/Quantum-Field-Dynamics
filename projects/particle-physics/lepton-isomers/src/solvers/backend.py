...
class BackendInterface:
    ...
    def fft(self, x, dims=None):
        """N-dimensional FFT."""
        raise NotImplementedError

    def ifft(self, x, dims=None):
...
class TorchBackend(BackendInterface):
    ...
    @property
    def xp(self):
        """Access to underlying tensor library."""
        return torch
    ...
    def ifft(self, x, dims=None):
        return torch.fft.ifft(x, dim=dims)
    ...
    def laplacian(self, x, K2):
        ...
        return self.real(self.ifft(-K2 * self.fft(x)))

    def gradient1d(self, x, dx):
        # Use central difference for interior points, forward/backward for boundaries
        grad = torch.zeros_like(x)
        grad[1:-1] = (x[2:] - x[:-2]) / (2 * dx)
        grad[0] = (x[1] - x[0]) / dx  # Forward difference at start
        grad[-1] = (x[-1] - x[-2]) / dx # Backward difference at end
        return grad
