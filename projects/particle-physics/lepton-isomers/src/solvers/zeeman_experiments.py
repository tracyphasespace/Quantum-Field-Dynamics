...
from .phoenix_solver import load_particle_constants, solve_psi_field
...
class ZeemanExperiment:
    ...
    def compute_g_factor(
        ...
        """
        logger.info(
            f"Computing g-factor: B={B_field:.3e} T, direction={field_direction}"
        )
        ...
        results = {
            ...
            "relative_correction": (
                float(Delta_g / self.g_classical) if self.g_classical != 0 else 0.0
            ),
        }
    ...
    def _create_magnetic_hamiltonian(
        self, B_field: float, direction: str
    ) -> PhoenixHamiltonian:
    ...
    def zeeman_field_sweep(
        self, B_fields: List[float], field_direction: str = "z"
    ) -> Dict[str, List]:
        """
        ...
        if len(B_fields) >= 2:
            ...
            g_extrapolated = self._extrapolate_g_factor(
                results["B_fields"], results["g_factors"]
            )
            results["g_factor_extrapolated"] = g_extrapolated
            results["delta_g_extrapolated"] = g_extrapolated - self.g_classical

        logger.info(
            f"Zeeman sweep completed: <g> = {np.mean(results['g_factors']):.8f}"
        )
        return results

    def _extrapolate_g_factor(
        self, B_fields: List[float], g_factors: List[float]
    ) -> float:
        ...
        return coeffs[1]  # Intercept = g-factor at B=0
...
class IsomerZeemanAnalysis:
    ...
    def __init__(
        ...
        logger.info(
            "IsomerZeemanAnalysis initialized for unified lepton g-2 predictions"
        )
    ...
    def calibrate_from_electron(self) -> Dict:
        ...
        g_factor = electron_results.get(
            "g_factor_extrapolated", np.mean(electron_results["g_factors"])
        )
        ...
        calibration = {
            ...
            "relative_error": abs(experimental_g2 - self.experimental_g2["electron"])
            / self.experimental_g2["electron"],
            "calibrated_physics": physics,
            "zeeman_results": electron_results,
        }

        logger.info(
            f"Electron calibration: g-2 = {experimental_g2:.10f} (error: {calibration['relative_error']*100:.2f}%)"
        )

        return calibration

    def predict_isomer_g2(
        self, calibration: Dict, target_mass_eV: float, isomer_name: str
    ) -> Dict:
        """
        ...
        if excited_state["converged"]:
            ...
            # Extract g-2
            g_factor = isomer_zeeman_results.get(
                "g_factor_extrapolated", np.mean(isomer_zeeman_results["g_factors"])
            )
            ...
            prediction = {
                ...
                "mass_error": abs(excited_state["final_energy"] - target_mass_eV)
                / target_mass_eV,
            ...
            logger.warning(
                f"Failed to predict {isomer_name} g-2: excited state not found"
            )
    ...
    def run_complete_isomer_analysis(self) -> Dict:
        ...
        results["muon_prediction"] = self.predict_isomer_g2(
            results["electron_calibration"], 105.6583745e6, "muon"  # Muon mass in eV
        )

        # Step 3: Tau prediction
        results["tau_prediction"] = self.predict_isomer_g2(
            results["electron_calibration"], 1776.86e6, "tau"  # Tau mass in eV
        )
    ...
    def _create_analysis_summary(self, results: Dict) -> Dict:
        ...
        summary = {
            ...
            "electron_calibrated": results["electron_calibration"].get(
                "relative_error", 1.0
            )
            < 0.1,
        ...
        if summary["electron_calibrated"]:
            summary["electron_g2"] = results["electron_calibration"][
                "electron_g2_predicted"
            ]
...
class ExcitedStateZeemanSolver:
    ...
    def find_excited_state(self, max_iterations: int = 20) -> Dict:
        ...
        for iteration in range(max_iterations):
            ...
            energy_error = (
                abs(results["H_final"] - self.target_energy) / self.target_energy
            )

            logger.info(
                f"Iteration {iteration}: E = {results['H_final']:.0f} eV, error = {energy_error:.4f}"
            )
        ...
        logger.warning(
            f"Excited state search incomplete: best error = {best_error:.4f}"
        )
        return {
            "converged": False,
    ...
    def zeeman_sweep_excited_state(
        self, excited_state: Dict, B_fields: List[float]
    ) -> Dict:
