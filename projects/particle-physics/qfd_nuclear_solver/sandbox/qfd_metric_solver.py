...
# Kernel configuration (sandbox experimentation)
KERNEL_PROFILE = os.environ.get("KERNEL_PROFILE", "ricker").lower()
KERNEL_MIX = float(os.environ.get("KERNEL_MIX", "0.5"))
KERNEL_INTEGRATION_POINTS = int(os.environ.get("KERNEL_POINTS", "5000"))
KERNEL_R_MAX_MULTIPLIER = float(os.environ.get("KERNEL_RMAX_FACTOR", "6.0"))
...
class QFDMetricSoliton:
    ...
    def _kernel_raw(self, r):
        """Unnormalized kernel, used for both Gaussian and Ricker components."""
        gaussian = math.exp(-r**2 / (2 * self.sigma_kernel**2))

        r2 = r**2
        ricker_core = 2.0 - (r2 / self.sigma_kernel**2)
        ricker = ricker_core * math.exp(-r2 / (2 * self.sigma_kernel**2)) if ricker_core > 0 else 0.0
        ...
        mix = min(max(self.kernel_mix, 0.0), 1.0)
        return (1.0 - mix) * gaussian + mix * ricker

    def _compute_kernel_norm(self):
        """
        Precompute normalization so each kernel integrates to 1 mass unit.
        Numeric integration handles all profile combinations uniformly.
        """
    ...
    def kernel_function(self, r):
        ...
        Two options exist in the sandbox:
          - Gaussian: legacy behavior
          - Ricker (Mexican hat, clipped to positive part) to embed gradient +
            Heaviside characteristics inside each blob
        """
    ...
    def analyze(self, result, M_exp_total, verbose=True):
...
def run_solver(isotopes=None, verbose=True):
    """Calculate nuclear masses for the provided isotopes."""
    if isotopes is None:
    ...
    for A, Z, name, M_exp in isotopes:
        ...
        M_model, E_stab = soliton.analyze(result, M_exp, verbose=verbose)
