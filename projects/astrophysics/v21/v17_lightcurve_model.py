...
K_J_BASELINE = 70.0  # km/s/Mpc (cosmic drag baseline)
...
class QFDIntrinsicModelJAX:
    ...
    def spectral_luminosity(
        ...
        # FIX 2025-01-16: Keep float64 precision! Don't cast to float32 (causes underflow)
        return jnp.maximum(jnp.nan_to_num(L_lambda_nm_64, nan=0.0, posinf=1e30, neginf=0.0), 1e-30)
...
def qfd_tau_total_jax(
    ...
    """
    Iteratively solve for total optical depth including flux-dependent photon self-interaction.

    Physical Model:
    - Plasma Veil: Photon-electron scattering in expanding plasma cloud
    - Photon Sear (FDR): Photon self-interaction in intense pulse

    τ_total = τ_plasma + τ_FDR
    τ_FDR = eta_prime * sqrt(flux_dimmed / flux_ref)

    NOTE: ξ (xi) is NOT used here! It models Thermal Broadening in Stage 2,
    which is a separate magnitude correction (delta_mu_thermal), not an opacity effect.
    ...
    Args:
        ...
        A_plasma: Plasma veil amplitude (scattering strength)
        beta: Wavelength exponent (selective scattering)
        eta_prime: FDR coupling parameter (photon self-interaction strength)
        tau_decay: Temporal decay timescale (plasma dissipation, ~30 days)
    ...
    def body_fn(i, val):
        ...
        exp_arg = jnp.clip(-tau_total, a_min=-80.0, a_max=0.0)
        flux_current = flux_lambda_geometric * jnp.exp(exp_arg)
        flux_normalized = jnp.maximum(flux_current / FLUX_REFERENCE, 0.0)
        # FIX 2025-01-16: Only eta_prime! (xi is for thermal broadening in Stage 2)
        tau_fdr = eta_prime * jnp.sqrt(flux_normalized + 1e-9)
    ...
    # FIX 2025-01-16: RE-ENABLED iterative FDR loop (was bypassed "for debugging")
    # Run 10 iterations to converge tau_fdr self-consistently
    initial_val = (tau_plasma, 0)
    tau_total, _ = jax.lax.fori_loop(0, 10, body_fn, initial_val)
...
def qfd_lightcurve_model_jax(
    ...
    global_params: Tuple[float, float],  # (k_J_correction, eta_prime) - xi removed!
    persn_params: Tuple[float, float, float, float],  # (t0, ln_A, A_plasma, beta)
    L_peak: float,
    z_obs: float,
) -> float:
    """
    V18 Stage 1 lightcurve model: Pure QFD + plasma + FDR, NO time-varying BBH lensing.

    Physics (see Physics.md):
    - Cosmic Drag: Distance-dependent redshift (k_J_correction)
    - Plasma Veil: Photon-electron scattering opacity (A_plasma, beta)
    - Photon Sear (FDR): Photon self-interaction opacity (eta_prime)

    NOTE: ξ (xi) is NOT in global_params! It's used only in Stage 2 for
    thermal broadening magnitude correction in ln_A_pred(), not in flux calculation.

    Args:
        obs: [t_obs (MJD), wavelength_obs (nm)]
        global_params: (k_J_correction, eta_prime) - 2 parameters only!
    ...
    """
    k_J_correction, eta_prime = global_params  # FIX 2025-01-16: xi removed!
    ...
    # FIX 2025-01-16: Plasma veil is OPACITY effect (tau_plasma), NOT redshift
    # Removed z_plasma from redshift calculation to fix double-counting bug
    # Plasma dimming now handled solely via tau_plasma in qfd_tau_total_jax()

    # NO BBH gravitational redshift in Stage 1
    z_bbh = 0.0

    # Total redshift - cosmological only (no plasma, no BBH)
    # FIX 2025-01-16: Removed z_plasma term (now pure opacity effect)
    z_total = z_cosmo

    # Rest-frame wavelength - no local effects in Stage 1
    # FIX 2025-01-16: Removed z_plasma (Plasma Veil is opacity, not redshift)
    wavelength_rest = wavelength_obs
    ...
    # FIX 2025-01-16: xi removed (used for thermal broadening in Stage 2, not opacity)
    ...
    # FIX 2025-01-18: Use consistent units for wavelength conversion
    # flux_lambda_dimmed is in erg/s/cm²/nm, so use λ in nm and c in nm/s
    wavelength_nm = jnp.clip(wavelength_obs, a_min=100.0, a_max=1000.0)  # Reasonable optical range
    C_NM_S = C_CM_S * 1e7  # Convert cm/s to nm/s: 2.998e10 cm/s * 1e7 nm/cm = 2.998e17 nm/s
    flux_nu = flux_lambda_dimmed * wavelength_nm**2 / C_NM_S  # erg/s/cm²/Hz
    flux_jy = flux_nu / 1e-23  # Convert to Janskys (1 Jy = 10^-23 erg/s/cm²/Hz)
...
@jit
def qfd_lightcurve_model_jax_with_stretch(
    obs: jnp.ndarray,  # [t_obs, wavelength_obs]
    global_params: Tuple[float, float],  # (k_J_correction, eta_prime) - xi removed!
    persn_params: Tuple[float, float, float, float, float],  # (t0, ln_A, stretch, A_plasma, beta)
    L_peak: float,
    z_obs: float,
) -> float:
    """
    V18 Stage 1 lightcurve model WITH STRETCH: Pure QFD + plasma + FDR + time-axis stretch.
    ...
    - Plasma Veil: Photon-electron scattering opacity (A_plasma, beta)
    - Photon Sear (FDR): Photon self-interaction opacity (eta_prime)
    - Time Stretch: Allows light curve width to vary (NEW!)

    Args:
        obs: [t_obs (MJD), wavelength_obs (nm)]
        global_params: (k_J_correction, eta_prime) - 2 parameters only!
        persn_params: (t0, ln_A, stretch, A_plasma, beta)  ← stretch added!
        L_peak: Peak bolometric luminosity (erg/s)
        z_obs: Observed heliocentric redshift
    ...
    Returns:
        Predicted flux in Jy (monochromatic approximation at wavelength_obs)
    """
    k_J_correction, eta_prime = global_params
    t0, ln_A, stretch, A_plasma, beta = persn_params  # ← stretch unpacked here
    t_obs, wavelength_obs = obs

    # Time since explosion (observer frame)
    t_since_explosion = t_obs - t0

    # Apply stretch to time axis - this is the KEY TEST
    # ΛCDM: If time dilation exists, stretch should ∝ (1+z)
    # QFD: If no time dilation, stretch should be constant
    t_rest = t_since_explosion / stretch  # ← STRETCH APPLIED HERE
    ...
    # Total redshift - cosmological only
    z_total = z_cosmo

    # Rest-frame wavelength - no local effects in Stage 1
    wavelength_rest = wavelength_obs

    # Get intrinsic luminosity at stretched rest-frame time
    L_intrinsic = QFDIntrinsicModelJAX.spectral_luminosity(
        t_rest,  # ← Uses stretched time
        wavelength_rest,
        L_peak,
        t_rise=19.0,
        temp_peak=12000.0,
    ...
    # Geometric flux at the fiducial distance
    D_L_fid_cm = D_fiducial_mpc * 3.0857e24
    ...
    # Plasma + FDR opacity (no BBH lensing)
    _, flux_lambda_dimmed = qfd_tau_total_jax(
        t_since_explosion,  # Note: plasma uses observer time, not stretched
        wavelength_obs,
        flux_lambda_geometric,
        A_plasma,
        beta,
...
def qfd_lightcurve_model_jax_static_lens(
    ...
    # Total redshift (multiplicative composition) - cosmological + BBH only
    # FIX 2025-01-16: Removed z_plasma term (now pure opacity effect)
    z_total = (1.0 + z_cosmo) * (1.0 + z_bbh) - 1.0

    # Rest-frame wavelength with BBH gravity corrections only
    # FIX 2025-01-16: Removed z_plasma term (Plasma Veil is opacity, not redshift)
    wavelength_rest = wavelength_obs / (1.0 + z_bbh)
