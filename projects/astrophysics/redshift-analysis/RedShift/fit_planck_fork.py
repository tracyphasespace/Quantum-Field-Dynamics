...
def model_spectra(ells, params, chi_star, sigma_chi, sigma_phase, phi0):
    ...
    Cee = fE * Ctt
    rho = te_correlation_phase((ells + 0.5)/chi_star, rpsi, ells, chi_star, sigma_phase=sigma_phase, phi0=phi0)
    Cte = rho * np.sqrt(Ctt * Cee)
    return Ctt, Cte, Cee

def lnlike(theta, data_df, chi_star, sigma_chi, sigma_phase, phi0, which="TT"):
    ells = data_df["ell"].values
    Ctt, Cte, Cee = model_spectra(ells, theta, chi_star, sigma_chi, sigma_phase, phi0)
    if which=="TT":
        model = Ctt
...
def main():
    ...
    ap.add_argument("--sigma_phase", type=float, default=0.16)
    ...
    def lnprob(th):
        ...
        if not (0.9<ns<1.05 and 80<rpsi<240 and 0.0<=Aosc<=1.0 and 0.001<sigma_osc<0.2 and 0.01<fE<1.0):
            return -np.inf
        return lnlike(th, df, args.chi_star, args.sigma_chi, args.sigma_phase, args.phi0, which=args.which)
