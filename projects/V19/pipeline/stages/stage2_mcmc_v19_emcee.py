...
from v19_lightcurve_model import ln_A_pred
from v19_data import LightcurveLoader


def load_real_data(lightcurve_path: str, stage1_dir: str, max_sne: int = 50, sntype_filter: list = None) -> Dict[str, np.ndarray]:
    """
    Same logic as stage2_mcmc_v18.py, but returning NumPy arrays for emcee.
    V19: Added sntype_filter for type-stratified MCMC.
    """
    print(f"Loading data for up to {max_sne} supernovae...")
    if sntype_filter is not None:
        print(f"  Filtering for SNTYPE: {sntype_filter}")
    loader = LightcurveLoader(Path(lightcurve_path))
    all_lcs = loader.load(sntype_filter=sntype_filter)

    z_obs_list, ln_A_obs_list, A_plasma_list, beta_list = [], [], [], []
    sntype_list = []  # V19: Track SNTYPE for each SN
    ...
    for snid, lc_data in sorted(all_lcs.items()):
        ...
        try:
            ...
            sntype_list.append(lc_data.sntype)  # V19: Collect SNTYPE
    ...
    # V19: Print SNTYPE distribution
    if sntype_list:
        unique_types, counts = np.unique(sntype_list, return_counts=True)
        print(f"  SNTYPE distribution:")
        for stype, count in zip(unique_types, counts):
            print(f"    SNTYPE {stype}: {count} SNe")
    ...
    return {
        ...
        "sntype": np.array(sntype_list, dtype=np.int32),  # V19: Include SNTYPE
    }
...
def main():
    parser = argparse.ArgumentParser(description="V19 Stage 2 MCMC (ln_A basis, emcee)")
    parser.add_argument(
        "--lightcurves",
        type=str,
    ...
    parser.add_argument(
        "--out", type=str, default="v19/results/stage2_emcee_lnA", help="Output dir"
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="V19: Only use SNe with these SNTYPE values (e.g., 0 1 4 for Type Ia)"
    )
    args = parser.parse_args()

    global z_obs, ln_A_obs
    data = load_real_data(args.lightcurves, args.stage1_results, args.max_sne, args.sntype_filter)
    z_obs = data["z_obs"]
    ...
    # V19: Add SNTYPE distribution to summary
    if "sntype" in data:
        sntype_array = data["sntype"]
        unique_types, counts = np.unique(sntype_array, return_counts=True)
        summary["sntype_distribution"] = {
            int(stype): int(count) for stype, count in zip(unique_types, counts)
        }
        summary["total_sne"] = len(sntype_array)
