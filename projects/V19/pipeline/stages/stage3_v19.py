...
"""
Stage 3: Distance Modulus Calculation & Hubble Diagram for V19 Pipeline

V19 Multi-Type: Color-coded by SNTYPE with type-stratified statistics.
...
# Add core directory to path for importing v19 models
sys.path.insert(0, str(Path(__file__).parent.parent / "core"))
from v19_lightcurve_model import ln_A_pred, K_J_BASELINE, qfd_lightcurve_model_jax_static_lens
from v19_data import LightcurveLoader
...
def process_single_sn(result, k_J_correction, eta_prime, xi):
    ...
    try:
        ...
        return {
            ...
            'sntype': result.get('sntype', 0),  # V19: Pass through SNTYPE
            'success': True
        }
    ...
    for result_dir in Path(stage1_dir).iterdir():
        ...
        try:
            ...
            result = {
                ...
                'iters': metrics['iters'],
                'z': lc.z,
                'sntype': lc.sntype  # V19: Include SNTYPE for type-colored plots
            }
...
def main():
    parser = argparse.ArgumentParser(description='Stage 3: V19 Multi-Type Hubble Diagram')
    ...
    print("=" * 80)
    print("STAGE 3: V19 HUBBLE DIAGRAM - MULTI-TYPE (MULTIPROCESSING)")
    print("=" * 80)
    print()
    ...
    with open(csv_file, 'w') as f:
        f.write("snid,z,alpha,mu_obs,mu_qfd,mu_lcdm,residual_qfd,residual_lcdm,residual_ln_A,chi2_per_obs,sntype\n")
        for d in data:
    ...
    # V19: Compute SNTYPE distribution and type-stratified statistics
    sntype_arr = np.array([d['sntype'] for d in data])
    unique_types, counts = np.unique(sntype_arr, return_counts=True)
    sntype_distribution = {int(stype): int(count) for stype, count in zip(unique_types, counts)}

    # V19: Type-stratified RMS
    type_stratified_stats = {}
    for stype in unique_types:
        mask = sntype_arr == stype
        if np.sum(mask) > 0:
            type_stratified_stats[int(stype)] = {
                'n_sne': int(np.sum(mask)),
                'qfd_rms': float(np.std(res_qfd_arr[mask])),
                'lcdm_rms': float(np.std(res_lcdm_arr[mask])),
                'median_z': float(np.median(z_arr[mask]))
            }
    ...
    with open(summary_file, 'w') as f:
        json.dump({
            ...
            'sntype_distribution': sntype_distribution,  # V19: SNTYPE counts
            'type_stratified_stats': type_stratified_stats,  # V19: Per-type RMS
    ...
    # V19: Print type-stratified statistics
    print("Type-stratified statistics:")
    for stype in sorted(type_stratified_stats.keys()):
        stats = type_stratified_stats[stype]
        print(f"  SNTYPE {stype}: N={stats['n_sne']}, "
              f"QFD RMS={stats['qfd_rms']:.3f} mag, "
              f"ΛCDM RMS={stats['lcdm_rms']:.3f} mag, "
              f"median z={stats['median_z']:.2f}")
    print()

    # Create plots
    print("Creating plots...")

    # V19: Define color map for SNTYPE
    type_colors = {
        0: '#1f77b4',   # Blue - Type Ia unclassified
        1: '#2ca02c',   # Green - Type Ia spec-confirmed
        4: '#ff7f0e',   # Orange - Type Ia peculiar
        23: '#d62728',  # Red - Type IIP
        29: '#9467bd',  # Purple - Type IIn
        32: '#8c564b',  # Brown - Type Ib
        33: '#e377c2',  # Pink - Type Ic
        39: '#7f7f7f',  # Gray - Type II
        41: '#bcbd22',  # Yellow-green - SLSN-I
        66: '#17becf',  # Cyan - SLSN-II
        129: '#aec7e8', # Light blue - Type IIn
        139: '#ffbb78', # Light orange - Type II
        141: '#98df8a'  # Light green - SLSN
    }
    ...
    # Main Hubble diagram - color-coded by SNTYPE
    for stype in unique_types:
        mask = sntype_arr == stype
        color = type_colors.get(stype, '#999999')  # Gray for unknown types
        label = type_labels.get(stype, f'SNTYPE {stype}')
        ax1.scatter(z_arr[mask], mu_obs_arr[mask], alpha=0.6, s=20,
    ...
    ax1.plot(z_model, mu_lcdm_model, 'k--', label='ΛCDM', linewidth=2, alpha=0.7)

    ax1.set_ylabel('Distance Modulus μ', fontsize=12)
    ax1.set_title(f'V19 Hubble Diagram - Multi-Type (N={len(data)} SNe)', fontsize=14)
    ax1.legend(fontsize=8, loc='upper left', ncol=2)
    ax1.grid(True, alpha=0.3)

    # Residuals - color-coded by SNTYPE (QFD only for clarity)
    for stype in unique_types:
        mask = sntype_arr == stype
        color = type_colors.get(stype, '#999999')
        label = type_labels.get(stype, f'SNTYPE {stype}')
        ax2.scatter(z_arr[mask], res_qfd_arr[mask], alpha=0.6, s=20,
                   c=color, edgecolors='none')

    ax2.axhline(0, color='black', linestyle='--', alpha=0.5)

    # Add sigma bands
    sigma = np.std(res_qfd_arr)
    ax2.axhline(sigma, color='gray', linestyle=':', alpha=0.5, label=f'±1σ ({sigma:.3f} mag)')
    ax2.axhline(-sigma, color='gray', linestyle=':', alpha=0.5)

    ax2.set_xlabel('Redshift z', fontsize=12)
    ax2.set_ylabel('QFD Residual (mag)', fontsize=12)
    ax2.set_title('V19 QFD Hubble Residuals (Multi-Type)', fontsize=14)
