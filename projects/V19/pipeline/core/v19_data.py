"""
V19 Data Loading - Multi-Type Supernova Support

Extended from V17 to handle all supernova types (not just Type Ia).
Adds SNTYPE column support and type-stratification capabilities.
"""
...
@dataclass
class SupernovaData:
    """Single supernova lightcurve data (V19 multi-type)."""
    ...
    # V19: Supernova type classification
    sntype: int = 0  # SNTYPE from DES-SN5YR (0=unclassified, 1=Ia, etc.)
...
class LightcurveLoader:
    ...
    def load(
        ...
        sntype_filter: Optional[List[int]] = None,
    ) -> Dict[str, SupernovaData]:
        """
        Load lightcurves from CSV (V19 multi-type).

        Parameters
        ----------
        n_sne : int, optional
            Maximum number of SNe to load
        start_sne : int
            Starting index for SN selection
        z_min, z_max : float, optional
            Redshift range filter
        require_peak : bool
            Require peak coverage (not implemented)
        sntype_filter : List[int], optional
            V19: Only include SNe with SNTYPE in this list
            Example: [0, 1, 4] for Type Ia only
                     [23, 29, 32, 33, 39] for core-collapse only

        Returns
        -------
        Dict[str, SupernovaData]
            Dictionary mapping SNID to SupernovaData objects
        """
        ...
        # V19: Filter by SNTYPE if requested
        if sntype_filter is not None and 'sntype' in df.columns:
            sn_types = df.groupby('snid')['sntype'].first()
            sn_types_filtered = sn_types[sn_types.isin(sntype_filter)]
            unique_snids = np.intersect1d(unique_snids, sn_types_filtered.index.values)
            print(f"  After SNTYPE filter {sntype_filter}: {len(unique_snids)} SNe")
        ...
        for snid in unique_snids:
            ...
            try:
                ...
                # V19: Extract SNTYPE if available
                sntype = int(sn_df['sntype'].iloc[0]) if 'sntype' in sn_df.columns else 0
