...
def filter_all_types(
    header_table: Table,
    include_all_types: bool = True,
    exclude_types: list = None
) -> np.ndarray:
    """
    Filter for all supernova types (V19 multi-type analysis).
    ...
    include_all_types : bool
        If True, include all SNTYPE values
    exclude_types : list, optional
        List of SNTYPE values to exclude (e.g., [80, 81, 82] for AGN/TDE/M-star)

    Returns
    -------
    np.ndarray
        Boolean mask for selected SNe (True for all by default)
    """
    sntype = header_table['SNTYPE'].data

    if include_all_types:
        # Include ALL types (default for V19)
        mask = np.ones(len(sntype), dtype=bool)

        # Optionally exclude specific types (e.g., non-SN transients)
        if exclude_types is not None:
            for exclude_type in exclude_types:
                mask &= (sntype != exclude_type)
    else:
        # Fallback: Type Ia only (V18 behavior)
...
def process_des_sn5yr_dataset(
    ...
    """
    Process DES-SN5YR dataset and create unified CSV (V19 multi-type version).
    ...
    include_all_types : bool
        Include all SNTYPE values (default True for V19)
    exclude_types : list, optional
        List of SNTYPE values to exclude (e.g., [80,81,82] for AGN/TDE)
    min_observations : int
    ...
    -------
    pd.DataFrame
        Unified light curve dataframe with SNTYPE column
    """
    # Locate FITS files
    ...
    combined_mask = type_mask & quality_mask

    if verbose:
        print(f"  SNe passing type filter: {type_mask.sum()}")
        print(f"  Quality cuts pass: {quality_mask.sum()}")
        print(f"  Final selection: {combined_mask.sum()}")

        # Print SNTYPE distribution
        selected_sntypes = header_table['SNTYPE'][combined_mask].data
        unique_types, type_counts = np.unique(selected_sntypes, return_counts=True)
        print(f"\n  SNTYPE distribution:")
        for sntype_val, count in zip(unique_types, type_counts):
            print(f"    SNTYPE={sntype_val}: {count} SNe ({count/len(selected_sntypes)*100:.1f}%)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Extract raw DES-SN5YR photometry (V19 multi-type)")
    ...
    parser.add_argument("--include-all-types", action="store_true", default=True,
                        help="Include all SNTYPE values (V19 default)")
    parser.add_argument("--exclude-types", type=int, nargs='+', default=None,
                        help="SNTYPE values to exclude (e.g., --exclude-types 80 81 82)")
