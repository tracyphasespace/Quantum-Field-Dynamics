# V18.1 Controlled Experiment: Critical Physics Fixes

## Overview

This project contains TWO fundamental physics corrections to the QFD supernova analysis pipeline that invalidate all previous V18 and V19 results. This is a **controlled experiment** to establish the first scientifically valid QFD baseline using clean Type Ia supernovae.
...
**Fix**: Enabled self-consistent iterative solver (20 iterations) in `pipeline/core/v17_lightcurve_model.py:393`:
```python
# BEFORE (THE BUG):
...
**Fix**: Pure opacity model in `pipeline/core/v17_lightcurve_model.py:535,626`:
```python
# BEFORE (THE BUG):
...
### 3. Generate Dataset
```bash
cd /path/to/Quantum-Field-Dynamics/projects/V18.1
python scripts/extract_full_dataset.py \
    --output data/lightcurves_type_ia_clean.csv \
...
**Verify dataset**:
```bash
wc -l data/lightcurves_type_ia_clean.csv
# Should show: 649683 (649682 data rows + 1 header)

head -n 2 data/lightcurves_type_ia_clean.csv
```
...
**Expected outputs**:
- `results/stage1_optimized_params.csv`: Per-SN best-fit parameters (6,895 rows)
- `results/stage1_residuals.csv`: Residuals for each observation
...
**Verification**:
```bash
wc -l results/stage1_optimized_params.csv
# Should show: 6896 (6895 SNe + 1 header)

# Check that optimization succeeded
head -n 5 results/stage1_optimized_params.csv
```
...
**Expected outputs**:
- `results/stage2_mcmc_chains.h5`: MCMC chains (HDF5 format)
- `results/stage2_corner_plot.png`: Corner plot showing parameter correlations
- `results/stage2_summary.txt`: Parameter posteriors (mean ± std)
...
python -c "import h5py; f=h5py.File('results/stage2_mcmc_chains.h5', 'r'); print('Chains shape:', f['chain'].shape); f.close()"
...
**Expected outputs**:
- `results/stage3_hubble_diagram.png`: Hubble diagram plot
- `results/stage3_residuals_qfd.png`: QFD residuals vs. redshift
- `results/stage3_residuals_lcdm.png`: ΛCDM residuals vs. redshift
- `results/stage3_statistics.txt`: Chi-squared comparison
...
├── scripts/
│   └── extract_full_dataset.py       # Dataset extraction from FITS
├── pipeline/
│   ├── core/
│   │   └── v17_lightcurve_model.py    # CRITICAL PHYSICS FIXES APPLIED
│   └── stages/
│       ├── stage1_optimize_v17.py     # Per-SN optimization module
│       ├── stage2_mcmc_v18_emcee.py   # MCMC global parameters
│       └── stage3_v18.py              # Hubble diagram generation
...
**Date**: 2025-11-18
**Repository**: https://github.com/tracyphasespace/Quantum-Field-Dynamics/tree/main/projects/V18.1
