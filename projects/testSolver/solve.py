...
def main(argv: List[str]) -> int:
    ...
    if not os.path.isdir(schema_dir):
        # Legacy layout: schema file sits directly beside solve.py
        schema_dir = os.path.dirname(os.path.abspath(__file__))

    skip_validation = os.environ.get("TESTSOLVER_SKIP_VALIDATION", "").strip() in ("1", "true", "TRUE")
    if skip_validation:
        print("[testSolver] Skipping schema validation (TESTSOLVER_SKIP_VALIDATION set)", file=sys.stderr)
    else:
