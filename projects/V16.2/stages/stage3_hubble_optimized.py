...
# Add core directory to path for importing v15_model
...
def process_single_sn(result, k_J, eta_prime, xi):
    ...
    not ln_A_obs, so residuals are meaningful (not zero).
    """
    try:
        z = result['z']
        ...
        ln_A_th = float(ln_A_pred_batch(np.array([z]), k_J, eta_prime, xi)[0])
...
def main():
    ...
    # ============================================================================
    # The ln_A values from Stage 1 have an arbitrary zero-point (from ln_A_guess).
    # Stage 2 MCMC fits the shape vs. redshift but doesn't know the absolute scale.
    # We calibrate by finding the offset that centers the residuals at zero.
