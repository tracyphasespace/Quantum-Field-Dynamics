...
class ZeemanBundleWrapper:
    ...
    def dE_dB_to_anomaly(self, dE_dB: float) -> float:
        g_factor = self._constants.get("g_factor")
...
def load_bundle_data(manifest_path: Path) -> Dict[str, Any]:
    ...
    extras_rel = manifest.get("extras")
    if extras_rel:
...
def extract_all_features(bundle: Dict[str, Any], chi_mode: str) -> Dict[str, float]:
    ...
    try:
        ...
        if chi_mode == "q_over_I":
            _need(bundle["features"], ["q_star","I"], chi_mode)
            chi = Q / I
        elif chi_mode == "qR_over_I":
            _need(bundle["features"], ["q_star","R_eff","I"], chi_mode)
            chi = (Q * R_eff) / I
        elif chi_mode == "LdotQ_over_I":
            _need(bundle["features"], ["L_mag","q_star","I"], chi_mode)
            L_mag, _ = extract_feature(bundle, "L_mag", "L_mag", 8)
            chi = abs(L_mag) * Q / I
        elif chi_mode == "hcsr_over_I":
            _need(bundle["features"], ["Hcsr","I"], chi_mode)
...
def main() -> None:
    ...
    # Load bundles
    try:
        muon_b = load_bundle_data(Path(args.muon))
        tau_b  = load_bundle_data(Path(args.tau))
        ele_b  = load_bundle_data(Path(args.electron)) if args.electron else None
    ...
    if args.emit_zeeman:
        ...
        try:
            # Muon Zeeman probe
            muon_zeeman_wrapper = ZeemanBundleWrapper(muon_b)
            ...
            # Tau Zeeman probe
            tau_zeeman_wrapper = ZeemanBundleWrapper(tau_b)
            ...
            if ele_b:
                electron_zeeman_wrapper = ZeemanBundleWrapper(ele_b)
    ...
    try:
        for exps in exp_tuples:
            mu = extract_all_features(muon_b, args.chi_mode)
            ta = extract_all_features(tau_b,  args.chi_mode)
            el: Optional[Dict[str, float]] = None

            # Gather chi values; include electron for normalization if provided
            chis = [mu["chi"], ta["chi"]]
            if ele_b:
                el = extract_all_features(ele_b, args.chi_mode)
                chis.append(el["chi"])
        ...
        if ele_b:
            ...
            electron_features = extract_all_features(ele_b, args.chi_mode)
