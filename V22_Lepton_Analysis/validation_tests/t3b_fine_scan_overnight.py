#!/usr/bin/env python3
"""
T3b STRATEGY 4: Fine Beta Scan at Lambda Sweet Spot

Based on analysis of initial sweep showing:
  - λ = 0: Best fit but β unidentified (CV = 95%)
  - λ = 1e-09: Best β identification (CV = 35%)
  - λ > 1e-08: Over-constrained

This run maps χ²(β, λ) at high resolution in the sweet spot region.

Configuration:
  - Lambda: [0, 5e-10, 1e-09, 2e-09] (4 values)
  - Beta: 1.7 to 2.7, step 0.1 (11 points)
  - Workers: 4 (for 8GB systems)
  - n_starts: 3 (balanced)

Expected: 4 λ × 11 β × 6 min = ~4.5 hours, ~6 GB peak memory
"""
...
print("=" * 80)
print("T3b STRATEGY 4: Fine Beta Scan at Sweet Spot")
print("=" * 80)
print()
print("Objective: High-resolution χ²(β, λ) mapping")
print()
print("Configuration rationale:")
print("  - Lambda sweet spot: 0 to 2e-09 (balanced fit vs identification)")
print("  - Beta refined grid: 1.7 to 2.7 (where optimal values cluster)")
print("  - 11 × 4 = 44 combinations for definitive landscape")
print()

log_memory("Initial")

# SWEET SPOT lambda grid (focused on 1e-10 to 1e-09 region)
LAM_CURV_GRID = [0.0, 5e-10, 1e-09, 2e-09]

print(f"Lambda sweep: {len(LAM_CURV_GRID)} values (sweet spot)")
print(f"  {LAM_CURV_GRID}")
print()

# FINE beta grid: 11 points from 1.7 to 2.7 (step 0.1)
beta_grid = np.arange(1.7, 2.75, 0.1)

print(f"FINE β grid: {len(beta_grid)} points from {beta_grid[0]:.1f} to {beta_grid[-1]:.1f} (step 0.1)")
print(f"  {beta_grid}")
print()
...
# Initial guess from best fit at λ=0, β=1.9
x_init = np.array([0.5912, 0.0086, 1.4134, 0.7973])
...
N_STARTS = 3      # Balanced multi-start
WORKERS = 4       # Use more cores if memory available
POPSIZE_MULT = 12 # popsize = 12*4 = 48
MAXITER = 200

print("OPTIMIZATION SETTINGS (8GB budget):")
print(f"  n_starts:     {N_STARTS}")
print(f"  workers:      {WORKERS}")
print(f"  popsize:      {POPSIZE_MULT}*n_params = {POPSIZE_MULT*4}")
print(f"  maxiter:      {MAXITER}")
print()
print(f"Expected per beta: ~6 min")
print(f"Expected per lambda: {len(beta_grid)} × 6 min = ~{len(beta_grid)*6} min = {len(beta_grid)*6/60:.1f} hr")
print(f"Total runtime: {len(LAM_CURV_GRID)} λ × {len(beta_grid)*6/60:.1f} hr = ~{len(LAM_CURV_GRID)*len(beta_grid)*6/60:.1f} hr")
print()
sys.stdout.flush()
...
def save_lambda_results(lam_curv, results_scan, append=True):
    ...
    # Append or create
    csv_path = "results/V22/t3b_fine_scan_full_data.csv"
    if append:
        try:
    ...
    summary_path = "results/V22/t3b_fine_scan_summary.csv"
    if append:
        try:
...
def fit_multi_start_optimized(fitter, x_prev, bounds, maxiter=200, base_seed=0, workers=4, n_starts=3):
    """Multi-start with optimized settings"""
    results = []
    chi2s = []
...
for lam_idx, lam_curv in enumerate(LAM_CURV_GRID):
    ...
    # Start from middle of beta grid
    ...
    for i in tqdm(range(idx_start, len(beta_grid)), desc=f"  β scan (asc, λ={lam_curv:.1e})", unit="β", leave=False):
        ...
        result = fit_multi_start_optimized(fitter, x_prev, bounds, maxiter=MAXITER, base_seed=42,
    ...
    # Clear memory
    del results_scan, best, S_opts, Cg_opts, chi2_values
    gc.collect()
...
print("  - results/V22/t3b_fine_scan_full_data.csv")
print("  - results/V22/t3b_fine_scan_summary.csv")
print()
log_memory("Final")
