#!/usr/bin/env python3
"""
T1: e,μ Regression with WIDENED BOUNDS + EXTENDED β RANGE

Purpose: Test whether bound saturation and β-edge pinning are boxing artifacts.

Configuration:
  - k = 1.5, Δv/Rv = 0.5, p = 6 (outside-only localization)
  - E_total = E_circ + E_stab + E_grad (all penalties add)
  - TIER A WIDENED BOUNDS (see below)
  - EXTENDED β: [1.5, 3.4] to capture lower edge behavior
...
Stop/Go criteria:
  STOP (boxing artifact falsified) if:
    - β_min still at lower edge AND χ² ~ 1e8 AND parameters cornered
  GO (artifact confirmed) if:
    - β_min interior OR χ² drops ≥100× OR bound hits collapse
"""
...
print("=" * 80)
print("T1: e,μ REGRESSION (Widened Bounds + Extended β)")
print("=" * 80)
print()
...
print(f"  Bounds: Tier A (widened)")
print(f"  β range: EXTENDED to capture lower edge")
print()
...
class TwoLeptonFitterCorrected:
    ...
    def fit(self, max_iter=200, seed=None):
        ...
        # Tier A widened bounds (per V22 test plan)
        bounds = [
            (0.15, 3.00),   # R_c_e (was 0.5-1.5)
            (0.005, 0.40),  # U_e (was 0.01-0.10)
            (0.05, 0.999),  # A_e (was 0.70-1.0)
            (0.02, 1.20),   # R_c_mu (was 0.05-0.30)
            (0.01, 0.60),   # U_mu (was 0.05-0.20)
            (0.05, 0.999),  # A_mu (was 0.70-1.0)
        ]
        ...
        result = differential_evolution(
            ...
            atol=1e-8,
            tol=1e-8,
            workers=8,  # Parallel evaluation on 8 cores
        )
...
# ========================================================================
# β Scan (EXTENDED to capture lower edge)
# ========================================================================

beta_range = (1.5, 3.4)
n_beta = 39  # ~0.05 spacing

print(f"β scan: [{beta_range[0]}, {beta_range[1]}] with {n_beta} points (EXTENDED)")
print("  Rationale: Previous runs pegged β at 3.0 lower edge")
print("  Must extend lower to determine if β is truly monotone or has interior minimum")
print()
...
for beta in tqdm(beta_grid, desc="β scan", unit="point"):
    ...
    result = fitter.fit(max_iter=100, seed=42)  # Keep 100 for T1 initial exploration
...
with open("results/V22/t1_widened_bounds_results.json", "w") as f:
    json.dump(results_dict, f, indent=2)

print("Results saved: results/V22/t1_widened_bounds_results.json")
print()
print("=" * 80)
