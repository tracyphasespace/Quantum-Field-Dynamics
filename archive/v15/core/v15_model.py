...
def ln_A_pred(z, k_J_correction, eta_prime, xi):
    ...
    k_J_total = K_J_BASELINE + k_J_correction  # 70.0 + correction
    ln_A_baseline = 2.0 * jnp.log(k_J_total / K_J_BASELINE)
...
class QFDIntrinsicModelJAX:
    ...
    def spectral_luminosity(
        ...
        T_eff = jnp.clip(T_eff, a_min=1000.0, a_max=50000.0)  # Physical temperature bounds

        # Planck function B_λ(T)
        wavelength_cm = jnp.clip(wavelength_nm * 1e-7, a_min=1e-8, a_max=1e-4)  # Reasonable wavelength range
...
def qfd_lightcurve_model_jax(
    ...
    """
    k_J_correction, eta_prime, xi = global_params  # k_J_total = 70.0 + k_J_correction
    ...
    k_J_total = K_J_BASELINE + k_J_correction  # 70.0 + correction
    D_fiducial_mpc = z_obs * C_KM_S / k_J_total  # Hubble-like relation
    ...
    # A = exp(ln_A) absorbs geometric dimming, calibration, and propagation
...
def log_likelihood_single_sn_jax_studentt(
    ...
    Student-t log-likelihood per observation:
        log L_i = log Γ((ν+1)/2) - log Γ(ν/2) - 0.5 log(ν π σ²)
                  - ((ν+1)/2) log(1 + residual²/(ν σ²))
