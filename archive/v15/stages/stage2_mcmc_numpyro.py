...
def log_likelihood_ln_A_space(
    ...
    *,
    ...
    Uses ln_A_pred(z; k_J_correction, eta_prime, xi) to compute residuals.
    k_J_correction is correction to k_J baseline (k_J_total = 70.0 + k_J_correction).
    No per-SN parameters, no lightcurve physics - just alpha prediction.

    Args:
        k_J_correction: Correction to k_J baseline (km/s/Mpc)
        cache_bust: Static cache-busting token to force fresh JIT compilation
    """
    # Predict alpha from globals
    ln_A_th = ln_A_pred_batch(z_batch, k_J_correction, eta_prime, xi)
...
def numpyro_model_ln_A_space(Phi, ln_A_obs_batch, *,
    ...
    if constrain_signs == 'informed':
        # Variant D: Informed priors centered at reasonable values
        # k_J_correction: Small correction to baseline k_J = 70.0 km/s/Mpc
        # Paper values for local effects: η' ≈ -8.0, ξ ≈ -7.0
        ...
        # k_J_correction is correction to 70.0 baseline, so center at 0
        k_J_correction = numpyro.sample('k_J_correction', dist.Normal(0.0, 10.0))
        ...
        c = jnp.array([k_J_correction, eta_prime, xi]) * scales  # FIXED: removed incorrect negative sign
        ...
        # Sample physics params
        # k_J_correction can be positive or negative (correction to 70.0 baseline)
        # eta_prime and xi assumed positive for this variant
        k_J_correction = numpyro.sample('k_J_correction', dist.Normal(0.0, 10.0))
        ...
        means = jnp.array(standardizer.means)
        scales = jnp.array(standardizer.scales)
        c = jnp.array([k_J_correction, eta_prime, xi]) * scales  # FIXED: using k_J_correction
...
def numpyro_model(persn_params_batch, L_peaks, photometries, redshifts, batch_size=500):
    ...
    """
    # Priors: k_J_correction is small correction to baseline (70.0)
    # η′ and ξ from paper's values: η′≈-8, ξ≈-7 (but using wide ranges)
    k_J_correction = numpyro.sample('k_J_correction', dist.Normal(0.0, 10.0))
...
def main():
    ...
    if args.use_ln_a_space and args.constrain_signs == 'off':
        ...
        k_J_correction_samples = c_samples[:, 0] / scales[0]
        ...
        k_J_correction_samples = np.asarray(samples['k_J_correction'])
    ...
    print("=" * 80)
    for name, arr in [('k_J_correction', k_J_correction_samples), ('eta_prime', eta_prime_samples),
                       ('xi', xi_samples)]:
    ...
    k_J_correction_best = float(np.median(k_J_correction_samples))
    ...
    k_J_correction_std = float(np.std(k_J_correction_samples))
    eta_prime_std = float(np.std(eta_prime_samples))
    xi_std = float(np.std(xi_samples))

    # Compute k_J_total from correction (k_J_total = 70.0 + k_J_correction)
    K_J_BASELINE = 70.0  # km/s/Mpc from QVD baseline cosmology
    k_J_total_samples = K_J_BASELINE + k_J_correction_samples
    k_J_total_best = float(np.median(k_J_total_samples))
    k_J_total_std = float(np.std(k_J_total_samples))

    print("=" * 80)
    print("MCMC RESULTS")
    print("=" * 80)
    print(f"Best-fit parameters (median ± std):")
    print(f"  k_J_correction = {k_J_correction_best:.2f} ± {k_J_correction_std:.4f} km/s/Mpc")
    print(f"  k_J_total = {k_J_total_best:.2f} ± {k_J_total_std:.4f} km/s/Mpc  (70.0 + correction)")
    ...
    if args.constrain_signs == 'physics':
        ...
        boundary_tol = 0.01
        k_J_correction_boundary_frac = np.mean(k_J_correction_samples < boundary_tol)
        ...
        print(f"    k_J:    {k_J_correction_boundary_frac*100:.1f}%")
        print(f"    η':     {eta_boundary_frac*100:.1f}%")
        print(f"    ξ:      {xi_boundary_frac*100:.1f}%")

        if max(k_J_correction_boundary_frac, eta_boundary_frac, xi_boundary_frac) > 0.1:
    ...
    samples_dict = {
        'params': ['k_J_correction', 'eta_prime', 'xi'],
        'samples': np.column_stack([k_J_correction_samples, eta_prime_samples, xi_samples]).tolist(),
        'mean': [float(np.mean(k_J_correction_samples)), float(np.mean(eta_prime_samples)), float(np.mean(xi_samples))],
        'median': [k_J_correction_best, eta_prime_best, xi_best],
        'std': [k_J_correction_std, eta_prime_std, xi_std],
    ...
    best_fit = {
        'k_J_correction': k_J_correction_best,
        'k_J_total': k_J_total_best,
        'eta_prime': eta_prime_best,
        'xi': xi_best,
        'k_J_correction_std': k_J_correction_std,
        'k_J_total_std': k_J_total_std,
    ...
    np.save(outdir / 'k_J_correction_samples.npy', k_J_correction_samples)
    np.save(outdir / 'k_J_total_samples.npy', k_J_total_samples)
