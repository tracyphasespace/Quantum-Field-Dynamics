...
def chi2_with_ridge(
    ...
    # Stage 1 fixes k_J at 70.0, so k_J_correction = 0.0
    # global_params from caller is (eta_prime, xi), but v15_model expects (k_J_correction, eta_prime, xi)
    global_params_with_kJ = (0.0,) + global_params  # Prepend k_J_correction = 0.0

    log_L = log_likelihood_single_sn_jax(global_params_with_kJ, persn_tuple, L_peak, phot, z_obs)
    chi2 = -2.0 * log_L
    ...
    def objective(params_jax):
        ...
        if use_studentt:
            ...
            log_L = log_likelihood_single_sn_jax_studentt(
                global_params_with_kJ, persn_tuple, L_peak, phot, z_obs, nu
            )
