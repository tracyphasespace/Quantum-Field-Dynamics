#!/usr/bin/env python3
"""Phase-10 SCF solver with symmetry energy term."""
import argparse
import json
import math
import os
...
class Phase11Model(torch.nn.Module):
    def __init__(self, A:int, Z:int, grid:int, dx:float,
        ...
        self.c_repulsion = float(c_repulsion)
    ...
    def energies(self) -> Dict[str, torch.Tensor]:
        ...
        # START SYMMETRY ENERGY TERM (QFD-specific surface effect)
        charge_asymmetry_sq = (self.A - 2 * self.Z)**2
        A13 = max(1.0, self.A)**(1.0/3.0)
        V_sym = torch.tensor(self.c_sym * charge_asymmetry_sq / A13, device=self.device)
        # END SYMMETRY ENERGY TERM

        # START SELF-REPULSION TERM
        V_repulsion = torch.tensor(self.c_repulsion * (self.A**(5.0/3.0)), device=self.device)
        # END SELF-REPULSION TERM

        return dict(
            V_repulsion=V_repulsion,
    ...
    def virial(self, energies: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        potential = (energies["V4_N"] + energies["V6_N"] +
                     ...
                     energies["V_iso"] + energies["V_rotor"] +
                     energies["V_surf"] + energies["V_coul_cross"] +
                     energies["V_sym"] +
                     energies["V_repulsion"] +
...
def scf_minimize(model: Phase10Model, iters_outer:int=360, lr_psi:float=1e-2, lr_B:float=1e-2,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Phase-10 SCF solver with symmetry energy")
    ...
    p.add_argument("--c-sym", type=float, default=0.0, help="Symmetry energy coefficient.")
    p.add_argument("--c-repulsion", type=float, default=0.0, help="Self-repulsion coefficient.")
    return p.parse_args()
...
def main() -> None:
    ...
    try:
        ...
        payload.update({k: energy_terms.get(k) for k in [
            ...
            "V_iso","V_rotor","V_surf","V_coul_cross","V_sym","V_repulsion","V_mass_N","V_mass_e"
        ]})
